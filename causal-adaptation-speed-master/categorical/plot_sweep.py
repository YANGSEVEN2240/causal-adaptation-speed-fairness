...
def fairness_all_plot(dense=True, input_dir='fairness_results', output_dir='fairness_plots',
                      figsize=(3.6, 2.2)):
    """Plot fairness experiment results."""
    init_type = 'denseinit' if dense else 'sparseinit'
    print(f'Fairness plots: {init_type}')

    for intervention in ['A', 'X', 'AX', 'Y']:
        filename = f'fairness_{intervention}_ka=5_kx=5_ky=5.pkl'
        if dense:
            filename = 'denseinit_' + filename
        else:
            filename = 'sparseinit_' + filename
        ...
        if os.path.isfile(filepath):
            ...
            nsteps = 100
            plotname = f'fairness_{intervention}_{init_type}'
            two_plots(results, nsteps, plotname=plotname,
                      dirname=output_dir, figsize=figsize)
        else:
